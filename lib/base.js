(function (window) {
    ...
    class HeadTH {
        ...
        getTH() {
            if (this.elements.hasOwnProperty('th')) return this.elements.th;
            let inside = this.getInside(), name = this.getMatrixName();
            this.elements.th = document.createElement('th');
    ...
    class Head {
        ...
        constructor(infinite) {
            ...
            this.elements = {};
            this.elements.th = [];
            this.elements.checkbox = new HeadCheckbox(this);
        }
        ...
        build() {
            ...
            for (let item = 0, th = this.emptyTH().getTH(); item < structure.length; item++) {
                let column = new HeadTH(this, structure[item]);
                th.push(column);
            }
    ...
    class BodyTD {
        ...
        build() {
            ...
            let value = this.getValue(),
                exposed = value === null || typeof value === 'undefined' ? Text.void() : value,
                element = matrix.handling.call(this, exposed);
    ...
    class BodyTR {
        ...
        constructor(body, matrix) {
            ...
            this.elements = {};
            this.elements.td = [];
            this.elements.checkbox = new BodyCheckbox(this);
        ...
        build() {
            ...
            if (structure.length !== 0) for (let item = 0, td = this.getTD(); item < structure.length; item++) {
                let nullable = !matrix.hasOwnProperty(structure[item].name) || null === matrix[structure[item].name],
                    column = new BodyTD(this, nullable ? Text.void() : matrix[structure[item].name], structure[item]);
                td.push(column);
            }
    ...
    class Body {
        ...
        addTR(matrix) {
            try {
                ...
                tr[matrix[unique]] = new BodyTR(this, matrix);
        ...
        compare(line, comparison) {
            ...
            for (let item = 0; item < setting.length; item++) {
                ...
                let one_comparison = one.getValue().toString().toLowerCase(), two_comparison = two.getValue().toString().toLowerCase();
                let lower = one_comparison < two_comparison, greater = one_comparison > two_comparison;
    ...
    class Infinite {
        ...
        constructor() {
            ...
            this.elements.head = new Head(this);
            this.elements.body = new Body(this);

            this.elements.loader.scroll = new LoaderScroll(this);
            this.elements.notice = new Notice(this);
            ...
            this.elements.loader.preloader = new Preloader(this);
        }
        ...
        result() {
            ...
            let offset = this.getRequestOffset() + response[key].length;
            this.setRequestOffset(offset);

            let body = this.getBody();
            for (let item = 0; item < response[key].length; item++) body.addTR(response[key][item]);
            body.update();

            let container = this.getContainer();
            if (container === null) return;

            let count = this.getRequestCount();
            if ((response.hasOwnProperty('complete') && response.complete === false)
                ||  (response[key].length - count === 0 && container.scrollHeight - container.offsetHeight - container.scrollTop <= 0)) return this.request();
