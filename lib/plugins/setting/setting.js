(function (window) {
	...
	class Search {
		...
		constructor(setting) {
			...
			this.elements = {};
			this.elements.notfound = new NotFound(this);
		}
	...
	class CheckboxOrder {
		...
		setInput(checked) {
			let trigger = document.createEvent('HTMLEvents'), input = this.getInput();
	...
	class Li {
		...
		constructor(ul, matrix) {
			this.ul = ul;
			this.elements = {};
			this.elements.order = new CheckboxOrder(this);
			this.elements.visibility = new CheckboxVisibility(this);
	...
	class Ul {
		...
		findLI(name) {
			let li = this.getLI();
			for (let item = 0; item < li.length; item++) {
				let data = li[item].out().getAttribute(Li.data());
				if (data === null
					|| data !== name) continue;
	...
	class Setting {
		...
		constructor(infinite) {
			...
			this.elements = {};
			this.elements.ul = new Ul(this);
			this.elements.search = new Search(this);
